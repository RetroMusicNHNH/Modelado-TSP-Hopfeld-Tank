"""Semana 3 - Integración con Runge-Kutta 4, instancia de 4 ciudades.

Repite el experimento de la semana 2 con RK4, que reproduce la trayectoria
continua con mayor fidelidad que Euler para el mismo tamaño de paso.
...
def main() -> None:
    ...
    print(f"== {instance.name} | método=rk4 ==")
    ...
    result = rk4_integrate(u0, d, params, record_every=100)
    ...
    plot_energy(result.times, result.energies,
                title="RK4 - Energía E(t), 4 ciudades",
                save_path=FIGURES / "energy" / "week3_rk4_4cities.png")
    plot_activation_matrix(result.V_final, labels=instance.labels,
                           title="RK4 - Matriz V final, 4 ciudades",
                           save_path=FIGURES / "matrices" / "week3_rk4_4cities.png")
    plot_route(instance.coords, route, labels=instance.labels,
               title=f"RK4 - Ruta {route_as_cycle_string(route, instance)}",
               save_path=FIGURES / "routes" / "week3_rk4_4cities.png")

    (DATA / "trajectories").mkdir(parents=True, exist_ok=True)
    (DATA / "final_states").mkdir(parents=True, exist_ok=True)
    np.savez(DATA / "trajectories" / "week3_rk4_4cities.npz",
             times=result.times, energies=result.energies, V_history=result.V_history)
    np.savez(DATA / "final_states" / "week3_rk4_4cities.npz",
